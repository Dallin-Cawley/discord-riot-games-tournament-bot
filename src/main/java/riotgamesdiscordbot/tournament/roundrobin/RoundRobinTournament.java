...
public class RoundRobinTournament extends Tournament {
    ...
    public RoundRobinTournament(TournamentConfig tournamentConfig, MessageReceivedEvent event,
                                Map<String, List<String>> teams, DiscordUser creator) {
        super(tournamentConfig, event, teams, creator);
        this.bracketManager = new RoundRobinBracketManager(this.teams,
                this.eventManager.getDiscordLogger().getTournamentImageChannel());
        this.currentRound = 1;
    }
    ...
    public void endTournament() {
        MessageEmbed messageEmbed = new TournamentWinnerEmbeddedMessageBuilder(this.tournamentWinner)
                .buildMessageEmbed();
        MessageCreateData data = MessageCreateData.fromEmbeds(messageEmbed);
        this.context.sendMessage(data).queue();
        this.isDone = true;
    }
